...
L0_Value* eval_string(const char* input) {
    reset_error_state();
    l0_arena_reset(test_arena); // Reset arena for this evaluation
...
void test_self_evaluating() {
    ...
    // L0_Value* expected_nil = l0_make_nil(test_arena); // Don't create expected value before arena reset
    // ASSERT_NOT_NULL(expected_nil);
    // ASSERT_TRUE(l0_is_nil(expected_nil));
    // assert_eval_eq_value("'()", expected_nil); // This comparison is flawed due to arena reset
    assert_eval_eq_nil("'()"); // Check the result type directly after eval_string resets arena
}

// Refactored test_quote to avoid issues with arena reset invalidating expected values
void test_quote() {
    printf("\n--- Testing Quote ---\n");
    // Don't create expected values here anymore due to arena reset in eval_string

    // Test 'hello
    {
        const char* input = "'hello";
        L0_Value* result = eval_string(input); // Arena reset happens here
        ASSERT_NOT_NULL(result);
        L0_Value* expected_sym = l0_make_symbol(test_arena, "hello"); // Create expected AFTER reset
        printf("Testing: %s => ", input); print_value(expected_sym); printf(" ... ");
        if (!values_equal(result, expected_sym)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184); // Use fixed line number for now
             fprintf(stderr, "  Expected: "); print_value(expected_sym); printf("\n");
             fprintf(stderr, "  Actual:   "); print_value(result); printf("\n");
             exit(1);
         }
         printf("OK\n");
    ...
    // Test (quote world)
    {
        const char* input = "(quote world)";
        L0_Value* result = eval_string(input);
        ASSERT_NOT_NULL(result);
        L0_Value* expected_sym = l0_make_symbol(test_arena, "world");
        printf("Testing: %s => ", input); print_value(expected_sym); printf(" ... ");
        if (!values_equal(result, expected_sym)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected_sym); printf("\n");
    ...
    // Test '123
     {
        ...
        printf("Testing: %s => ", input); print_value(expected_int); printf(" ... ");
        if (!values_equal(result, expected_int)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected_int); printf("\n");
             fprintf(stderr, "  Actual:   "); print_value(result); printf("\n");
             exit(1);
         }
         printf("OK\n");
    ...
    // Test (quote 456)
    {
        const char* input = "(quote 456)";
        L0_Value* result = eval_string(input);
        ASSERT_NOT_NULL(result);
        L0_Value* expected_int = l0_make_integer(test_arena, 456);
        printf("Testing: %s => ", input); print_value(expected_int); printf(" ... ");
        if (!values_equal(result, expected_int)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected_int); printf("\n");
    ...
    // Test '(a 1)
    {
        ...
        printf("Testing: %s => ", input); print_value(expected_list); printf(" ... ");
        if (!values_equal(result, expected_list)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected_list); printf("\n");
             fprintf(stderr, "  Actual:   "); print_value(result); printf("\n");
             exit(1);
         }
         printf("OK\n");
     ...
     // Test (quote (b 2))
    {
        const char* input = "(quote (b 2))";
        L0_Value* result = eval_string(input);
        ASSERT_NOT_NULL(result);
        L0_Value* expected_list = l0_make_pair(test_arena, l0_make_symbol(test_arena, "b"), l0_make_pair(test_arena, l0_make_integer(test_arena, 2), l0_make_nil(test_arena)));
        printf("Testing: %s => ", input); print_value(expected_list); printf(" ... ");
        if (!values_equal(result, expected_list)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected_list); printf("\n");
    ...
    // Test ''a - Manually check structure to avoid arena/comparison issues
    {
        const char* input = "''a";
        printf("Testing: %s => (quote a) ... ", input);
        L0_Value* result = eval_string(input); // Arena reset happens here
        ASSERT_NOT_NULL(result);

        // Manually check the structure of (quote a)
        ASSERT_TRUE(l0_is_pair(result)); // Should be a pair
        if (!l0_is_pair(result)) { fprintf(stderr, " FAILED: result is not a pair\n"); exit(1); }

        L0_Value* car_val = l0_car(result);
        L0_Value* cdr_val = l0_cdr(result);
        ASSERT_NOT_NULL(car_val);
        ASSERT_NOT_NULL(cdr_val);

        ASSERT_TRUE(l0_is_symbol(car_val)); // Car should be symbol 'quote'
        if (!l0_is_symbol(car_val)) { fprintf(stderr, " FAILED: car is not a symbol\n"); exit(1); }
        ASSERT_TRUE(strcmp(car_val->data.symbol, "quote") == 0);
        if (strcmp(car_val->data.symbol, "quote") != 0) { fprintf(stderr, " FAILED: car is not 'quote'\n"); exit(1); }


        ASSERT_TRUE(l0_is_pair(cdr_val)); // Cdr should be pair '(a)'
         if (!l0_is_pair(cdr_val)) { fprintf(stderr, " FAILED: cdr is not a pair\n"); exit(1); }

        L0_Value* cadr_val = l0_car(cdr_val);
        L0_Value* cddr_val = l0_cdr(cdr_val);
        ASSERT_NOT_NULL(cadr_val);
        ASSERT_NOT_NULL(cddr_val);

        ASSERT_TRUE(l0_is_symbol(cadr_val)); // Car of cdr should be symbol 'a'
        if (!l0_is_symbol(cadr_val)) { fprintf(stderr, " FAILED: cadr is not a symbol\n"); exit(1); }
        ASSERT_TRUE(strcmp(cadr_val->data.symbol, "a") == 0);
         if (strcmp(cadr_val->data.symbol, "a") != 0) { fprintf(stderr, " FAILED: cadr is not 'a'\n"); exit(1); }

        ASSERT_TRUE(l0_is_nil(cddr_val)); // Cdr of cdr should be nil '()
        if (!l0_is_nil(cddr_val)) { fprintf(stderr, " FAILED: cddr is not nil\n"); exit(1); }
...
void test_primitives() {
    ...
    {
        const char* input = "(cons 1 2)";
        L0_Value* result = eval_string(input);
        ASSERT_NOT_NULL(result);
        L0_Value* expected = l0_make_pair(test_arena, l0_make_integer(test_arena, 1), l0_make_integer(test_arena, 2));
        printf("Testing: %s => ", input); print_value(expected); printf(" ... ");
        if (!values_equal(result, expected)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected); printf("\n");
    ...
    // assert_eval_eq_value("(cons 'a '())", l0_make_pair(test_arena, l0_make_symbol(test_arena, "a"), l0_make_nil(test_arena)));
     {
        const char* input = "(cons 'a '())";
        L0_Value* result = eval_string(input);
        ASSERT_NOT_NULL(result);
        L0_Value* expected = l0_make_pair(test_arena, l0_make_symbol(test_arena, "a"), l0_make_nil(test_arena));
        printf("Testing: %s => ", input); print_value(expected); printf(" ... ");
        if (!values_equal(result, expected)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected); printf("\n");
    ...
    // assert_eval_eq_value("(cdr (cons 'a '()))", l0_make_nil(test_arena)); // Use specific nil check
    assert_eval_eq_nil("(cdr (cons 'a '()))");
...
void test_lambda_and_apply() {
    ...
    // Refactor (identity '(1 2)) test
    {
        const char* input = "(identity '(1 2))";
        L0_Value* result = eval_string(input); // Arena reset
        ASSERT_NOT_NULL(result);
        L0_Value* expected_list = l0_make_pair(test_arena, l0_make_integer(test_arena, 1), l0_make_pair(test_arena, l0_make_integer(test_arena, 2), l0_make_nil(test_arena))); // Create expected AFTER reset
        printf("Testing: %s => ", input); print_value(expected_list); printf(" ... ");
        if (!values_equal(result, expected_list)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184); // Use fixed line number
             fprintf(stderr, "  Expected: "); print_value(expected_list); printf("\n");
             fprintf(stderr, "  Actual:   "); print_value(result); printf("\n");
             exit(1);
         }
...
void test_strings() {
    ...
    {
        const char* input = "(string->symbol \"abc\")";
        L0_Value* result = eval_string(input);
        ASSERT_NOT_NULL(result);
        L0_Value* expected = l0_make_symbol(test_arena, "abc");
        printf("Testing: %s => ", input); print_value(expected); printf(" ... ");
        if (!values_equal(result, expected)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected); printf("\n");
    ...
    {
        const char* input = "(string->symbol \"+\")";
        L0_Value* result = eval_string(input);
        ASSERT_NOT_NULL(result);
        L0_Value* expected = l0_make_symbol(test_arena, "+");
        printf("Testing: %s => ", input); print_value(expected); printf(" ... ");
        if (!values_equal(result, expected)) {
             fprintf(stderr, "\nASSERT FAILED: %s:%d: Values not equal!\n", __FILE__, 184);
             fprintf(stderr, "  Expected: "); print_value(expected); printf("\n");
...
void test_string_ops() {
    ...
    // string-length
    assert_eval_eq_int("(string-length \"\")", 0);
    assert_eval_eq_int("(string-length \"abc\")", 3);
    assert_eval_eq_int("(string-length \"hello world\")", 11);
    assert_eval_eq_int("(string-length (string-append \"a\" \"bc\"))", 3);
    assert_eval_error("(string-length 123)", L0_PARSE_ERROR_RUNTIME);
    assert_eval_error("(string-length)", L0_PARSE_ERROR_RUNTIME);
    assert_eval_error("(string-length \"a\" \"b\")", L0_PARSE_ERROR_RUNTIME);

    // string-ref
    // Remember: string-ref returns the character code as an integer
    assert_eval_eq_int("(string-ref \"abc\" 0)", (long)'a');
    assert_eval_eq_int("(string-ref \"abc\" 1)", (long)'b');
    assert_eval_eq_int("(string-ref \"abc\" 2)", (long)'c');
    assert_eval_eq_int("(string-ref \"hello world\" 6)", (long)'w');
    assert_eval_error("(string-ref \"abc\" 3)", L0_PARSE_ERROR_RUNTIME); // Index out of bounds
    assert_eval_error("(string-ref \"abc\" -1)", L0_PARSE_ERROR_RUNTIME); // Index out of bounds
    assert_eval_error("(string-ref 123 0)", L0_PARSE_ERROR_RUNTIME); // Not a string
    assert_eval_error("(string-ref \"abc\" #t)", L0_PARSE_ERROR_RUNTIME); // Index not integer
    assert_eval_error("(string-ref \"abc\")", L0_PARSE_ERROR_RUNTIME); // Missing index
    assert_eval_error("(string-ref)", L0_PARSE_ERROR_RUNTIME); // Missing args
    assert_eval_error("(string-ref \"a\" 1 2)", L0_PARSE_ERROR_RUNTIME); // Too many args

    // substring
    assert_eval_eq_string("(substring \"hello\" 0 5)", "hello");
    assert_eval_eq_string("(substring \"hello\" 1 4)", "ell");
    assert_eval_eq_string("(substring \"hello\" 0 0)", "");
    assert_eval_eq_string("(substring \"hello\" 5 5)", "");
    assert_eval_eq_string("(substring \"hello\" 1)", "ello"); // Optional end argument
    assert_eval_eq_string("(substring \"hello\" 0)", "hello");
    assert_eval_eq_string("(substring \"hello\" 5)", "");
    assert_eval_error("(substring \"hello\" 6)", L0_PARSE_ERROR_RUNTIME); // Start out of bounds
    assert_eval_error("(substring \"hello\" -1)", L0_PARSE_ERROR_RUNTIME); // Start out of bounds
    assert_eval_error("(substring \"hello\" 0 6)", L0_PARSE_ERROR_RUNTIME); // End out of bounds
    assert_eval_error("(substring \"hello\" 3 2)", L0_PARSE_ERROR_RUNTIME); // End < Start
    assert_eval_error("(substring 123 0 1)", L0_PARSE_ERROR_RUNTIME); // Not a string
    assert_eval_error("(substring \"abc\" #t 1)", L0_PARSE_ERROR_RUNTIME); // Start not integer
    assert_eval_error("(substring \"abc\" 0 #f)", L0_PARSE_ERROR_RUNTIME); // End not integer
    assert_eval_error("(substring \"abc\")", L0_PARSE_ERROR_RUNTIME); // Missing start
    assert_eval_error("(substring)", L0_PARSE_ERROR_RUNTIME); // Missing args
    assert_eval_error("(substring \"a\" 1 2 3)", L0_PARSE_ERROR_RUNTIME); // Too many args

    // number->string
    assert_eval_eq_string("(number->string 123)", "123");
    assert_eval_eq_string("(number->string 0)", "0");
    assert_eval_eq_string("(number->string -45)", "-45");
    assert_eval_eq_string("(number->string (+ 100 23))", "123");
    assert_eval_error("(number->string \"abc\")", L0_PARSE_ERROR_RUNTIME); // Not a number
    assert_eval_error("(number->string #t)", L0_PARSE_ERROR_RUNTIME); // Not a number
    assert_eval_error("(number->string)", L0_PARSE_ERROR_RUNTIME); // Missing arg
    assert_eval_error("(number->string 1 2)", L0_PARSE_ERROR_RUNTIME); // Too many args
}
...
int main() {
    ...
    test_recursion();
    test_strings();
    test_string_ops(); // Call the new test function
    test_io();
