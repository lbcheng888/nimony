...
char* l0_codegen_program(L0_Arena* arena, L0_Value* ast_list) {
    ...
    success &= sb_append_str(&sb,
        ...
        "    // --- NEW: Set exit_code based on L0 result --- \n"
        "    if (exit_code == 0) { // Only check L0 result if no C-level error occurred\n"
        "        if (last_result == NULL) { // Should not happen if exit_code is 0, but check\n"
        "            exit_code = 1; // Treat NULL result as error\n"
        "        } else if (l0_is_integer(last_result)) {\n"
        "            exit_code = (int)last_result->data.integer; // Use integer result as exit code\n"
        "        } else if (l0_is_symbol(last_result)) {\n"
        "            // Check for specific success/error symbols\n"
        "            const char* result_sym = last_result->data.symbol;\n"
        "            if (strcmp(result_sym, \"success\") == 0) {\n"
        "                exit_code = 0;\n"
        "            } else {\n"
        "                // Any other symbol indicates an error\n"
        "                exit_code = 1; // Use 1 for generic L0 errors\n"
        "                fprintf(stdout, \"L0 Error Symbol: %s\\n\", result_sym); fflush(stdout);\n" // Print the error symbol
        "            }\n"
        "        } else if (l0_is_boolean(last_result) && last_result->data.boolean) {\n"
        "             exit_code = 0; // Treat #t as success \n"
        "        } else {\n"
        "            // Treat any other type (pair, nil, #f, etc.) as an error for exit code\n"
        "            exit_code = 1;\n"
        "        }\n"
        "    }\n"
        "    // --- END NEW --- \n\n"
        "    fprintf(stdout, \"[DEBUG C main] Reached cleanup. final exit_code = %d\\n\", exit_code); fflush(stdout);\n" // Updated debug print
        "    l0_arena_destroy(arena); \n"
